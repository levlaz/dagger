...
Volume caching is especially useful when dealing with package managers such as `npm`, `maven`, `pip` and similar. Since these dependencies are usually locked to specific versions in the application's manifest, re-downloading them on every session is inefficient and time-consuming.
...
[Learn more about caching](../api/caching.mdx) and see an [example of how to use volume caching for dependencies](../api/cache-volumes.mdx).
